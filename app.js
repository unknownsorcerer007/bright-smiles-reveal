/* ==========================================
   ...
   v5: Clean rebuild — US keywords + SEO/GEO/AEO + optimized
   ========================================== */
...
// ==========================================
// KEYWORDS — US dental terminology, SEO-optimized
// ==========================================
const STAR_KEYWORDS = {
    1: {
...
// ==========================================
// JOKES — Rage bait, highly shareable, no repeats
// ==========================================
const JOKES = {
    spa: [
...
function goBack() {
    ...
    navigateTo(state.screenHistory.pop(), false);
}
...
function toggleVibe(chip, keyword) {
    ...
    if (idx > -1) {
        ...
        if (state.selectedVibes.length >= 2) { showToast('Max 2 experience aspects'); haptic('error'); return; }
...
function getUniqueJoke(category) {
    ...
    if (available.length === 0) { state.usedJokes.clear(); return pool[Math.floor(Math.random() * pool.length)]; }
...
dom.reviewText.addEventListener('input', () => {
    ...
    state.selectedKeywords.forEach(kw => {
        ...
        const found = new RegExp(`\\b${escapeRegex(kw)}\\b`, 'i').test(text);
        if (chip) chip.classList.toggle('found', found);
        if (!found) allFound = false;
    });
    if (len < 20) { dom.reviewQuality.textContent = 'Keep writing...'; dom.reviewQuality.className = 'quality-indicator'; }
    else if (len < 80) { dom.reviewQuality.textContent = 'Good start!'; dom.reviewQuality.className = 'quality-indicator good'; }
    else if (len < 150) { dom.reviewQuality.textContent = 'Looking great!'; dom.reviewQuality.className = 'quality-indicator good'; }
    else { dom.reviewQuality.textContent = 'Excellent detail! ⭐'; dom.reviewQuality.className = 'quality-indicator great'; }
    if (allFound && len >= 20) { dom.btnCopyReview.disabled = false; dom.copySubtext.textContent = 'Ready to copy!'; }
    else { dom.btnCopyReview.disabled = true; dom.copySubtext.textContent = !allFound ? 'Keywords missing' : 'Write a bit more...'; }
});
...
dom.btnGotoGoogle.addEventListener('click', () => { window.open(`https://search.google.com/local/writereview?placeid=${CONFIG.googlePlaceId}`, '_blank'); });

if (dom.btnSkipGoogle) { dom.btnSkipGoogle.addEventListener('click', () => { window.open(`https://search.google.com/local/writereview?placeid=${CONFIG.googlePlaceId}`, '_blank'); }); }

dom.btnVerifyClaim.addEventListener('click', () => { navigateTo('scratch'); setTimeout(() => setupScratchCard(), 50); });
...
function setupScratchCard() {
    ...
    if (revealCard) {
        ...
        revealCard.innerHTML = `<div class="dynamic-card"><div class="card-brand-row"><span>🦷</span><span>BRIGHT SMILES</span></div><div class="card-character"><img src="${state.cardImage}" alt="Dental character"></div><div class="card-joke">"${state.joke}"</div><div class="card-footer-row">${state.footer} • BRIGHT SMILES</div></div>`;
    }
    ...
    grad.addColorStop(0, '#1e3a5f'); grad.addColorStop(0.5, '#2563eb'); grad.addColorStop(1, '#1e3a5f');
...
function scratchStart(e) { isDrawing = true; scratchMove(e); }
...
function scratchMove(e) {
    ...
    scratchCtx.restore();
    scratchCheckCounter++;
    if (scratchCheckCounter % 3 === 0) checkScratchProgress();
}

function scratchEnd() { isDrawing = false; if (!state.isScratched) checkScratchProgress(); }
...
function checkScratchProgress() {
    ...
    if ((transparent / total) * 100 > 18) {
        state.isScratched = true;
        haptic('success');
        canvas.style.transition = 'opacity 0.4s ease-out';
        canvas.style.opacity = '0';
        spawnConfetti(40);
        setTimeout(() => { canvas.style.display = 'none'; dom.btnGoToShare.style.display = 'flex'; }, 400);
    }
}

dom.btnGoToShare.addEventListener('click', () => { initShareScreen(); navigateTo('share'); });
...
function initShareScreen() {
    ...
    if (preview) {
        ...
        preview.innerHTML = `<div class="dynamic-card"><div class="card-brand-row"><span>🦷</span><span>BRIGHT SMILES</span></div><div class="card-character"><img src="${state.cardImage}" alt="Dental character"></div><div class="card-joke">"${state.joke}"</div><div class="card-footer-row">${state.footer} • BRIGHT SMILES</div></div>`;
    }
}
...
dom.btnShareWhatsapp.addEventListener('click', () => {
    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out my patient smile card from ${CONFIG.clinicName}! 🦷🪥`)}`, '_blank');
});

dom.btnRestart.addEventListener('click', () => {
    Object.assign(state, { selectedStars: 0, selectedVibes: [], selectedMenu: [], selectedKeywords: [], cardTheme: '', cardImage: '', joke: '', footer: '', isCopied: false, isScratched: false, screenHistory: [] });
    dom.starBtns.forEach(b => { b.classList.remove('selected'); b.setAttribute('aria-checked', 'false'); });
    dom.ratingFeedback.textContent = 'Tap a star to begin';
    dom.ratingFeedback.classList.remove('highlight');
    dom.reviewText.value = '';
    dom.btnCopyReview.style.display = 'flex';
    const canvas = dom.scratchCanvas;
    canvas.style.display = 'block'; canvas.style.opacity = '1'; canvas.style.transition = 'none';
...
function pulseElement(el) { el.style.transform = 'scale(1.08)'; setTimeout(() => { el.style.transform = ''; }, 150); }
...
document.addEventListener('DOMContentLoaded', () => {
    ['char_spa.webp', 'char_vacation.webp', 'char_camera.webp'].forEach(src => { const img = new Image(); img.src = src; });
    initStarRating();
    navigateTo('stars', false);
});
